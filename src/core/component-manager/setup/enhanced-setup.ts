...
export class EnhancedSetup {
  ...
  private static setupEnhancedCards(data: SentimentData, timeframe: TimeFrame): { [key: string]: EnhancedCard } {
    ...
    try {
      enhancedCards.spy = this.setupEnhancedMarketCard('spy', 'S&P 500 (SPY)', indicators.spy, data, timeframe);
    } catch (error) {
      console.warn('Failed to setup enhanced SPY card:', error);
    }
    
    try {
      enhancedCards.qqq = this.setupEnhancedMarketCard('qqq', 'Nasdaq 100 (QQQ)', indicators.qqq, data, timeframe);
    } catch (error) {
      console.warn('Failed to setup enhanced QQQ card:', error);
    }
    
    try {
      enhancedCards.iwm = this.setupEnhancedMarketCard('iwm', 'Russell 2000 (IWM)', indicators.iwm, data, timeframe);
    } catch (error) {
    ...
    try {
      enhancedCards.vix = this.setupEnhancedCard('vix', {
        ...
        message: MessageGenerators.getVixMessage(indicators.vix.value, timeframe),
  ...
  private static setupEnhancedCard(key: string, data: CardData): EnhancedCard {
    const containerId = key === 'fearGreed' ? 'fear-greed-card-container' : `${key}-card-container`;
    return new EnhancedCard(containerId, data);
  }

  private static setupEnhancedMarketCard(key: string, title: string, indicator: any, data: SentimentData, timeframe: TimeFrame): EnhancedCard {
