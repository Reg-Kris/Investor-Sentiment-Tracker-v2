...
import {
  ...
  OptionsCard,
  FearGreedCard,
} from '../../../components/indicator-card';
import type { SentimentData, TimeFrame } from '../../../types/sentiment';

export class IndicatorSetup {
  static setupSentimentCluster(
    data: SentimentData,
    timeframe: TimeFrame,
  ): SentimentCluster | undefined {
  ...
  static setupIndicatorCards(data: SentimentData, timeframe: TimeFrame) {
    ...
    const fearGreedContainer = document.getElementById(
    ...
    // Market Cards
    cards.spy = this.setupMarketCard(
      'spy',
      'S&P 500 (SPY)',
      indicators.spy,
      timeframe,
    );
    cards.qqq = this.setupMarketCard(
      'qqq',
      'Nasdaq 100 (QQQ)',
      indicators.qqq,
      timeframe,
    );
    cards.iwm = this.setupMarketCard(
      'iwm',
      'Russell 2000 (IWM)',
      indicators.iwm,
      timeframe,
    ...
    const marketOptionsContainer = document.getElementById(
      'market-options-card-container',
    );
  ...
  private static setupMarketCard(
    key: string,
    title: string,
    indicator: any,
    timeframe: TimeFrame,
