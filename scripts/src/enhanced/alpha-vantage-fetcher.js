...
export class AlphaVantageFetcher {
  ...
  async fetchIntradayData(symbol, interval = '60min') {
    try {
      ...
      const intradayData = await this.alphaVantage.data.intraday(symbol, interval);
      ...
      console.error(`❌ Error fetching ${symbol} intraday from Alpha Vantage:`, error.message);
      return this.mock.getMockStockData(symbol);
    }
  }
  ...
  async fetchTechnicalIndicator(symbol, indicator, params = {}) {
    try {
      ...
      switch (indicator.toUpperCase()) {
        case 'SMA':
          result = await this.alphaVantage.data.sma(symbol, params.interval || 'daily', params.timePeriod || 20);
          break;
        case 'EMA':
          result = await this.alphaVantage.data.ema(symbol, params.interval || 'daily', params.timePeriod || 20);
          break;
        case 'RSI':
          result = await this.alphaVantage.data.rsi(symbol, params.interval || 'daily', params.timePeriod || 14);
          break;
        case 'MACD':
          result = await this.alphaVantage.data.macd(symbol, params.interval || 'daily');
          break;
        default:
