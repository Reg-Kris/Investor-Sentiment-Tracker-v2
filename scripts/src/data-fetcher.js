...
class MarketDataFetcher {
  constructor() {
    ...
    this.fearGreedFetcher = new FearGreedFetcher(
      this.httpClient,
      this.cacheManager,
      this.circuitBreaker,
      this.rapidApiKey,
    ...
    this.vixFetcher = new VixFetcher(
      this.httpClient,
      this.cacheManager,
      this.circuitBreaker,
      this.fredApiKey,
    );
    this.optionsFetcher = new OptionsFetcher(
      this.httpClient,
      this.cacheManager,
      this.circuitBreaker,
    );
  ...
  validateApiKeys() {
    ...
    console.log(
      `  Alpha Vantage: ${keyStatus.alphaVantage ? '✅' : '⚠️  demo'}`,
    );
    ...
    if (!keyStatus.alphaVantage && !keyStatus.fred && !keyStatus.rapidApi) {
      console.warn(
        '⚠️  All APIs using demo keys - expect rate limits and mock data',
      );
    }
  ...
  async fetchAllData() {
    ...
    try {
      ...
      results.options =
        results.options || MockDataProvider.getMarketOptionsData();
    }
  ...
  logDataSummary(results) {
    ...
    console.log(
      `  Circuit breakers active: ${this.circuitBreaker.getStatus().activeCircuits}`,
    );
  }
}
